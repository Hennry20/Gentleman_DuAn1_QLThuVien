...
namespace DuAn1_QLThuVien
{
    public partial class QLTheHoiVien : Form
    {
                    ...
                    using (SqlCommand check_MaND_Command = new SqlCommand(check_MaND_Query, conn))
                    {
                        ...
                        else
                        {
                            ...
                            using (SqlCommand command = new SqlCommand(insertQuery, conn))
                            {
                                if (DateTime.TryParseExact(ngayDangKy, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out ngayDK))
                                {
                                    command.Parameters.AddWithValue("@NgayDangKi", ngayDK);
                                }
                                else
                                {
                                    MessageBox.Show("Định dạng ngày tháng không hợp lệ. \nVui lòng nhập lại đúng định dạng \nNăm/tháng/ngày (dd/MM/yyyy)", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                ...
                                if (DateTime.TryParseExact(ngayHetHan, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out ngayHH))
                                {
                                    command.Parameters.AddWithValue("@NgayHetHan", ngayHH);
                                }
                                else
                                {
                                    MessageBox.Show("Định dạng ngày tháng không hợp lệ. \nVui lòng nhập lại đúng định dạng \nNăm/tháng/ngày (dd/MM/yyyy)", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                ...
                                {
                                    command.Parameters.AddWithValue("@MaThe", CbbMaThe.Text.Trim());
                                    command.Parameters.AddWithValue("@MaND", txtManguoidoc.Text.Trim());
                                    command.Parameters.AddWithValue("@TenHoiVien", txtTenHoiVien.Text.Trim());
                                    command.Parameters.AddWithValue("@DiaChi", txtDiaChi.Text.Trim());
                                    command.Parameters.AddWithValue("@DiemHoiVien", txtDiemHoiVien.Text.Trim());
        ...
        private void XoaHV(string idToDelete)
        {
            string connString = @"Data Source=HUYNHQUYTRUONG;Initial Catalog=QLThuVien;Integrated Security=True";
            string deleteQuery = "DELETE FROM TheHoiVien WHERE MaThe = @MaThe";
            using (SqlConnection conn = new SqlConnection(connString))
            {
                conn.Open();
                using (SqlCommand command = new SqlCommand(deleteQuery, conn))
                {
                    command.Parameters.AddWithValue("@MaThe", idToDelete);
                    command.ExecuteNonQuery();
        ...
        {
            DialogResult result = MessageBox.Show($"Bạn có chắc chắn muốn xóa mã hội viên = {CbbMaThe.Text.Trim()}?", "Xác nhận?", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (result == DialogResult.OK)
            {
                string idToDelete = CbbMaThe.Text.Trim();
                XoaHV(idToDelete);
            }
